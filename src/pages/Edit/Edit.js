...
class Edit extends Component {
    ...
    update = () => {
        Axios.put(`http://localhost:3001/api/profiles/${this.props.match.params.id}`, this.state)
    ...
    getData = () => {
        Axios.get(`http://localhost:3001/api/profiles/${this.props.match.params.id}`)
            .then((response) => {
                const { firstName, lastName, honorific, email, phone, password } = response.data;
                this.setState({
                    firstName,
                    lastName,
                    honorific,
